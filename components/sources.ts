...
export const SOURCES: Record<string, { name: string; desc: string; label: string; url?: string }> = {
  NSCA: {
    label: "NSCA",
  ...
  COVERAGE: {
    name: "Coverage taxonomy (regional hypertrophy + lengthened-position research)",
    desc: "Complete-coverage checklist: every movement pattern weekly, plus within-muscle regions (delt heads, triceps long head, hamstring functions, gastroc/soleus). Muscles are not one thing; the audit walks this list so gaps get caught proactively.",
    label: "Coverage",
  },
  ISSN: {
    name: "ISSN / ACSM sports nutrition position stands",
    desc: "Protein 1.6-2.2 g/kg/day split across 4+ feedings; carbs scaled to training load (3-5 g/kg lift days, 5-8 g/kg hard endurance); recovery meal within a couple hours; maintenance-or-above energy plus ~1,500 mg calcium while bone heals.",
    label: "Nutrition",
  },
};
...
export const SOURCE_TAG_RE = /\[(NSCA|SCHOENFELD|DANIELS|80\/20|CONCURRENT|KOOP|CSS|BONESTRESS|RECOVERY|JUDGMENT|COVERAGE|ISSN)(?:[^\]]*)\]/g;
